#Code for Figure S5 of "Empirically derived thermal thresholds of four coral species along the Red Sea"
...
Acr_chla<-lmer(Acr_chla_log ~ Temp*Site + (1|Geno/Site), data = Acr_phys)
...
Acr_phys$Site = factor(Acr_phys$Site,levels(Acr_phys$Site)[c(4,2,5)])
...
Acr_chla <- ggplot(data=Acr_phys, 
                  aes(x=Temp, y=Chl_cm2, label=Temp, fill=Site)) +
  ...
  theme(line= element_line(size = 1),
        ...
        panel.spacing = unit(3, "lines")) + xlab(label = "Temperature (°C)") + ylab(label = "Chl a (µg cm-2)")+
...
Poc_phys$Site = factor(Poc_phys$Site,levels(Poc_phys$Site)[c(4,2,5)])

# Colours-> Eilat, Al Wajh, Yanbu, AlFahal, Al Q, Obock
# "royalblue2", "darkgoldenrod1", "darkorange1", "red3", "darkorchid4", "springgreen1")

Poc_chla <- ggplot(data=Poc_phys, 
                  aes(x=Temp, y=Chl_cm2, label=Temp, fill=Site)) +
  scale_fill_manual(values = c("royalblue2", "darkgoldenrod1", "darkorange1"), drop = FALSE, name = "Site") +
  stat_boxplot(geom ='errorbar', width = 0.7, lwd=0.7) +
  geom_boxplot(width=0.7, lwd=0.7, fatten=1) +
...
Sty_phys$Site = factor(Sty_phys$Site,levels(Sty_phys$Site)[c(4,5,3)]) 

# Colours-> Eilat, Al Wajh, Yanbu, AlFahal, Al Q, Obock
# "royalblue2", "darkgoldenrod1", "darkorange1", "red3", "darkorchid4", "springgreen1")

Sty_chla <- ggplot(data=Sty_phys, 
                  aes(x=Temp, y=Chl_cm2, label=Temp, fill=Site)) +
...
Por_phys$Site = factor(Por_phys$Site,levels(Por_phys$Site)[c(5,1,3)]) 

# Colours-> Eilat, Al Wajh, Yanbu, AlFahal, Al Q, Obock
# "royalblue2", "darkgoldenrod1", "darkorange1", "red3", "darkorchid4", "springgreen1")

Por_chla <- ggplot(data=Por_phys, 
                  aes(x=Temp, y=Chl_cm2, label=Temp, fill=Site)) +
